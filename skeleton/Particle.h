...
public:
	...
	Particle(Vector3 pos = Vector3(0,0,0), Vector3 v = Vector3(1,1,1), 
		Vector3 a = Vector3(0,0,0), Vector4 c = Vector4(1,1,1,1), 
		double d = 0.99, float ml = -1, float dY = 0, float mass = 1e6, Shape geometry = Shape::SPHERE);
	~Particle();
	...
	void setGeometry(Shape geometry) {
		_geometry = geometry; DeregisterParticle(); initRenderItem();
	}
	//GETTERS
